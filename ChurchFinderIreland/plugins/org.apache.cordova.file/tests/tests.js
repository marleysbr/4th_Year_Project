...
exports.defineManualTests = function (contentEl, createActionButton) {
    ...
    if (fsRoots.hasOwnProperty(cordova.platformId)) {
        (fsRoots[cordova.platformId].split(',')).forEach(function (fs) {
            if (cordova.platformId === 'ios' && fs === 'private') {
                createActionButton("ChurchFinderIreland private URL (iOS)", function () {
                    clearLog();
                    testPrivateURL();
                }, 'button');
            } else {
    ...
    div = document.createElement('h3');
    div.appendChild(document.createTextNode('For ChurchFinderIreland private URL (iOS), the private URL (first blue URL in status box) ' +
